...
- Branding layout uses `AppShell` + `SiteHeader`: home `minimal` sticky header (nav only) + `SiteHeroBrand`; compact sticky header with logo + Clusters link; mobile nav via shadcn Sheet (`MobileNavMenu`, links in `lib/site-nav.ts`); no header bottom border; transparent `logo.png` uses `object-contain` (`logo-background.png` is unused reference).
